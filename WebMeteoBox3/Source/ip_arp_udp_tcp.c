...
uint16_t www_client_internal_datafill_callback(uint8_t fd){
        ...
        if (fd==www_fd){
                if (browsertype==0){
                        ...
                        len=fill_tcp_data(bufptr,len,client_urlbuf_var);
                        len=fill_tcp_data_p(bufptr,len,PSTR(" HTTP/1.1\r\nHost: "));
                        //len=fill_tcp_data_p(bufptr,len,client_hoststr); // 16.04.2014
						len=fill_tcp_data(bufptr,len,client_hoststr);
                        if (client_additionalheaderline_p){
                                len=fill_tcp_data_p(bufptr,len,PSTR("\r\nAuthorization: Basic ")); // 14.09.2014
                                //len=fill_tcp_data_p(bufptr,len,client_additionalheaderline_p);
								len=fill_tcp_data(bufptr,len,client_additionalheaderline_p);
                        }
                        len=fill_tcp_data_p(bufptr,len,PSTR("\r\nUser-Agent: box/1.1\r\nAccept: */*\r\n"));
						...
						//strcpy_P(client_postval,PSTR("5555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555555-@uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuP"));
                        len=fill_tcp_data(bufptr,len,client_postval);
                }
				//printf("client_postval=%s\r\n",client_postval);
				//printf("bufptr=%s\r\n",&bufptr[54]);
				//printf("xlen=%d\r\n",len);
                return(len);
        }
...
void client_http_post(const char *urlbuf_p, char *urlbuf_varpart,const char *hoststr, const char *additionalheaderline_p,char *postval,void (*callback)(uint16_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac, uint16_t port)
{
        if (!enc28j60linkup())return;
        client_urlbuf_p=urlbuf_p;
        client_hoststr=hoststr;
        ...
        browsertype=1;
		//printf("client_hoststr=%s\r\n",&client_hoststr);
        client_browser_callback=callback;
        //www_fd=client_tcp_req(&www_client_internal_result_callback,&www_client_internal_datafill_callback,80,dstip,dstmac);
		www_fd=client_tcp_req(&www_client_internal_result_callback,&www_client_internal_datafill_callback,port,dstip,dstmac);
}
#endif // WWW_client
        ...
        if ( buf[TCP_DST_PORT_H_P]==TCPCLIENT_SRC_PORT_H){
                ...
                if (tcp_client_state==2){
                        if ((buf[TCP_FLAGS_P] & TCP_FLAGS_SYN_V) && (buf[TCP_FLAGS_P] &TCP_FLAGS_ACK_V)){
								...
								//printf("client_tcp_datafill_callback=%d\r\n",len);
                                tcp_client_state=3;
