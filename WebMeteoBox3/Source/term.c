...
void readsingle()
{
  ...
  unsigned char scratchpad[9];
    ...
    OWI_DetectPresence(OWI_PIN_5);
    OWI_SkipRom(OWI_PIN_5);
    OWI_SendByte(DS18B20_CONVERT_T ,OWI_PIN_5);

    while (!OWI_ReadBit(OWI_PIN_5));

    OWI_DetectPresence(OWI_PIN_5);
    OWI_SkipRom(OWI_PIN_5);
    OWI_SendByte(DS18B20_READ_SCRATCHPAD, OWI_PIN_5);
    scratchpad[0] = OWI_ReceiveByte(OWI_PIN_5);
    scratchpad[1] = OWI_ReceiveByte(OWI_PIN_5);
